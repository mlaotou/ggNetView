#' @noRd
create_layout_circular_modules_equal_square2_layout <- function(
    graph_obj,
    r = 1,
    node_add = 8,
    anchor_dist = 10,
  ...
  square2_from_anchor <- function(cx, cy, info_df, r_step) {
  ...
  for (i in seq_len(n_mod)) {
    ...
    coords_i <- square2_from_anchor(cx, cy, info_df, r_step = r_step_i)
