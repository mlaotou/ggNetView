#' @noRd
create_layout_circular_modules_equal_star_layout <- function(
    graph_obj,
    r = 0.1,
    node_add = 7,
    anchor_dist = 10,
  ...
  star_from_anchor <- function(cx, cy, info_df, r_step) {
  ...
  for (i in seq_len(n_mod)) {
    ...
    coords_i <- star_from_anchor(cx, cy, info_df, r_step = r_step_i)
